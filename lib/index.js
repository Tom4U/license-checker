 ...
 * @method
 * @param  {object} sorted            The sorted JSON data from all packages.
 ...
 */
exports.asJson = function (sorted, customFormat) {
  var output;
  ...
  if (customFormat && Object.keys(customFormat).length > 0) {
    ...
    for (moduleKey in sorted) {
      if (sorted.hasOwnProperty(moduleKey)) {
        module = sorted[moduleKey];
        moduleName = sorted[moduleKey].name;
        ...
        for (moduleProp in module) {
          if (module.hasOwnProperty(moduleProp)) {
            ...
            if (customFormat.hasOwnProperty(moduleProp)) {
              output[moduleName][moduleProp] = sorted[moduleKey][moduleProp];
            }
          }
        }
      }
...
exports.asCSV = function (sorted, customFormat, csvComponentPrefix) {
  ...
  Object.keys(sorted).forEach(function (key) {
    ...
    if (customFormat && Object.keys(customFormat).length > 0) {
      ...
      lineArr.push(['"' + key + '"', '"' + (module.licenses || '') + '"', '"' + (module.repository || '') + '"']);
      line = lineArr.join(',');
    }
    text.push(line);
  });
